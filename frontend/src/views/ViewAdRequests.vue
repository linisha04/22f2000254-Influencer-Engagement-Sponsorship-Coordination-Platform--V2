...
<div v-if="getAdRequests && getAdRequests.length>0">
    ...
    <tbody>
      <tr v-for="ad in getAdRequests" :key="ad.id">
        ...
        <td class="table-warning">Ad Id : {{ ad.id }} <br> campaign_id : {{ ad.campaign_id }} <br> Ad name : {{ ad.name }}
        <br> influencer_id : {{ ad.influencer_id }} <br> requirements :  {{ ad.requirements }} <br> Ad Amount : {{ ad.amount }}
      <br> Ad status :  {{ ad.status }} <br> Req Created by : {{ ad.created_by }} <br>  Req sent to : {{ ad.sent_to }}
    <br> <button @click="updateCampaign(camp.id)">Update</button>  <button @click="deleteCampaign(camp.id)">Delete</button> </td>
...
.message-container {
    ...
    overflow-y: auto;   
    padding: 10px;      
    background-color: grey; /* Optional: background color */
}
